...
int main(int argc, char **argv)
{
  ...
  if (jFile.empty())
  {
    ...
    for (const auto & fileItems : root["results"])  //iterating the json_file
    {
      ...
      vector<string> myVec;
      if (fileItems["licenses"].size() > 1)  //checking if a file contains more than one license
      {
        for (const auto & license : fileItems["licenses"]) //iterating the license array
        {
      ...
      result = compatibilityFunc(myVec, lic_types, rule);
      if (json)
      {
    ...
    vector<tuple<string, string, string>> result;
    string name = "null";
    result = compatibilityFunc(vector<string>(allLic.begin(), allLic.end()), lic_types, rule);
    if (json)
    {
...
compatibilityFunc(vector<string> myVec, const string& lic_types, const string& rule) {
    ...
    for (unsigned int argn2 = (argn + 1); argn2 < vecSize; argn2++)
    {
      ...
      for (const auto & yml_rfile : rules["rules"]) //iterating the yml.rules
      {
        ...
        for (const auto & tag : yml_rfile)
        {
          ...
          if ((rule1 == 3) &&
              (main_type == type && sub_type == type2)) //rule1 for maintype and subtype
          {
            res.emplace_back(main_name, sub_name, ans);
            def++;
            continue;
          }
          if ((rule2 == 3) && (((main_name == name) || (main_name == name2)) &&
                               ((sub_name == name) || (sub_name == name2)))) //rule2 for mainname and subname
          {
            res.emplace_back(main_name, sub_name, ans);
            ++priority;
            def++;
          ...
          if ((rule3 == 3) && (priority == 0) && ((main_name == name) || (sub_name == name)) &&
              ((main_type == type2) || (sub_type == type2))) //rule3 for mainname and subtype
          {
            res.emplace_back(main_name, sub_name, ans);
            def++;
